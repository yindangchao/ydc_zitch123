package com.vanpro.zitech125.daemon.util;
...
public class IntentWrapper {
    ...
    static {
        ...
        if (Build.VERSION.SDK_INT >= 25/*Build.VERSION_CODES.N*/) {
            ...
            boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(MyApplication.getInstance().getPackageName());
            if (!ignoringBatteryOptimizations) {
                Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                dozeIntent.setData(Uri.parse("package:" + MyApplication.getInstance().getPackageName()));
