...
import com.vanpro.zitech125.daemon.receiver.WakeUpReceiver;
...
//    public static Subscription sSubscription;
    ...
    int onStart(Intent intent, int flags, int startId) {
        ...
        if (Build.VERSION.SDK_INT <= 25/*Build.VERSION_CODES.N*/) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) startService(new Intent(MyApplication.getInstance(), WorkNotificationService.class));
        }
        ...
        getPackageManager().setComponentEnabledSetting(new ComponentName(getPackageName(), LifecycleService.class.getName()),
...
//                    MyApplication.getInstance().sendBroadcast(new Intent(WakeUpReceiver.ACTION_CANCEL_JOB_ALARM_SUB));
//                }).subscribe(count -> {
